...
% Unir os dados de sem sepse e com sepse. Sabe-se que os primeiros 34 pacientes NÃO tem sepse. E que os 36 últimos TEM.
A = [Pss,Pcs]; 


%%%%%%%%%%%%%% SVD COM TODOS OS ATRIBUTOS %%%%%%%%%%%%%%%%%%%5
...
title('Espectro singular da matriz de expressão gênica' ,'antes a seleção de atributos (GSE12624)', 'FontSize', 14, 'FontWeight', 'bold');
...
% Plota esferas para o grupo controle (1:36) - Verde
scatter3(x(1:36), y(1:36), z(1:36), 100, greenColor, 'filled', ...
         'MarkerEdgeColor', 'k', 'MarkerFaceAlpha', 0.8);

% Plota esferas para os pacientes com sepse (37:70) - Laranja queimado
scatter3(x(37:70), y(37:70), z(37:70), 100, burntOrange, 'filled', ...
...
%title('Pacients Space of GSE12624' ,'Contribution of Singular Vectors', 'FontSize', 14, 'FontWeight', 'bold');
...
%%%%%%%%%%%%%% SVD COM 10 MAIORES E 10 MENORES %%%%%%%%%%%%%%%%%%%5

% seleciona somente os 10 primeiros e 10 últimos genes para Ar

b(1:34) = log(0.00001 / (1 - 0.00001));
b(37:70) = log(0.999999 / (1 - 0.99999));
[alpha,x] = resolve(A',b);
[valores, pos] = sort(alpha);

Ar = [A([pos(1:10) pos(end-9:end)], :)];
%alphar = Ar'\b;
...
%pr = num./(1+num);
%figure
%title('P após a seleção dos atributos')
%plot(pr, '*')
%grid on
%figure
%plot(alphar, '*')
%title('Valores dos pesos no modelo reduzido')
...
xlabel('Componente singular');
ylabel('Valor singular normalizado');
title('Espectro singular da matriz de expressão gênica' ,'após a seleção de atributos (GSE12624)', 'FontSize', 14, 'FontWeight', 'bold');
...
% Adiciona título e rótulos
%title('Pacients Space of GSE12624 (After Feature Selection)' ,'Contribution of Singular Vectors', 'FontSize', 14, 'FontWeight', 'bold');
xlabel('First Singular Vector Coefficient', 'FontSize', 12);
ylabel('Second Singular Vector Coefficient', 'FontSize', 12);
zlabel('Third Singular Vector Coefficient', 'FontSize', 12);
